...
def load_profile_collection(path, *, patch_profiles=True, nspace=None):
    ...
    if not os.path.isdir(path):
        raise OSError(f"Failed to load the profile collection. Path '{path}' is not a directory.")
    ...
    if not len(file_list):
        raise OSError(f"The directory '{path}' contains no startup files (mask '[0-9][0-9]*.py').")
        ...
        for file in file_list:
            try:
                ...
                if not os.path.isfile(file):
                    raise OSError(f"Startup file {file!r} was not found")
    ...
    for p in param_list:
        ...
        if (k in kwargs_in) and (v_min is not None or v_max is not None):
            ...
            try:
                ...
                msg = f"{msg}\n" if msg else msg
                msg += f"Parameter value is out of range: key='{k}', value='{kwargs_in[k]}': {ex}"
                match = False
...
def save_existing_plans_and_devices(
    *,
    ...
    if os.path.exists(file_path) and not overwrite:
        raise OSError(f"File '{file_path}' already exists. File overwriting is disabled.")
